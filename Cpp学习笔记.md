...
一个类可以继承自多个父类，他可以从多个基类继承数据和函数。定义一个派生类，我们使用一个类派生列表来指定有哪些基类 代码如下：
...
隐藏与重写的区别：在父类和子类中，函数名相同，参数相同，父类的同名函数没有virtual关键字，为隐藏。
...
{
public:
void funA() {cout << "funA():Heri" << endl;} //函数隐藏 因为不是虚函数
void funA()(int a) {cout << "funA(0)(int a):Heri" << a << endl;} //函数隐藏 参数不同
...
- 析构函数虽然函数名不同，但是也可以构成重写。因为编译器为了让析构函数实现多态，会将它们的名字都处理为destructor，这样就可以构成多态。
- 析构函数应该设置为虚函数，当析构一个指向子类的父类指针时，编译器可以根据虚函数表寻找到子类的析构函数进行调用，从而正确释放子类对象的资源。不然可能内存泄漏（部分已被占用内存未被释放，导致以后无法使用该内存）
- 当类之间具有继承关系时，在多态中应该将父类的析构函数设置为虚函数（重写析构函数）。让析构函数具有多态，此时父类指针delete时才会准确的调用到子类的析构函数。如果析构函数不被声明成虚函数，则编译器实施静态绑定，在删除指向子类的父类指针时，只会调用父类的析构函数而不调用子类析构函数，这样就会造成子类对象析构不完全造成内存泄漏。
